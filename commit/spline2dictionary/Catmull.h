...
class Catmull
{
    ...
    Catmull(const Catmull& c) {L = c.L; weight = c.weight; curv = c.curv; KNOTs = c.KNOTs; P = c.P; dP = c.dP; ddP = c.ddP; }
  	...
  	//void		set(std::vector<std::vector<float> >);
    	void 		UseOldSegments(Catmull );
    	float       	degrees( int );
    	float		approxLen( );
    	void		eval( int );
    	void		arcLengthReparametrization( float );
    	float       	curvature ( int );
    	void 		MoveKnot(int, Vector<float>, float );
    	void 		StraightenKnots(int, int, float);
    	void		Translate(int, int, Vector<float>, float );
...
/*
void Catmull::set(std::vector<std::vector<float> > points)
{
	int len = points.size();
	KNOTs.resize(len+2);
	KNOTs[0].x = points[0][0];
	KNOTs[0].y = points[1][0];
	KNOTs[0].z = points[2][0];
	for ( int i = 0; i<len; i++) {
		KNOTs[i+1].x = points[0][i];
		KNOTs[i+1].y = points[1][i];
		KNOTs[i+1].z = points[2][i];
	}
	KNOTs.back().x = points[0][len-1];
	KNOTs.back().y = points[1][len-1];
	KNOTs.back().z = points[2][len-1];
    	L = approxLen();

}
*/
void Catmull::UseOldSegments(Catmull old)
{
	P = old.P;
	dP = old.dP;
	ddP = old.ddP;
...
void Catmull::MoveKnot(int knot, Vector<float> move_direction, float magnitude) {
...
void Catmull::Translate(int StartKnot, int EndKnot, Vector<float> move_direction, float magnitude) {
	Vector<float> direction;
	direction.Set(move_direction.x, move_direction.y, move_direction.z);
	direction.Multiply(magnitude);
...
void Catmull::MoveKnotOrthogonal(int StartKnot,int EndKnot, float magnitude) {								//cannot be edge knots!
	std::vector<Vector<float> > OldKnots;
	Vector<float> baseline;
	Vector<float> knotvector;
	Vector<float> straightenvector;
