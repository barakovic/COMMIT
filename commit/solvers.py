...
def solve(y, A, At, tol_fun = 1e-4, tol_x = 1e-6, max_iter = 1000, verbose = 1, x0 = None, regularisation = None, save_x_interval = None):
    """
    ...
    return fista( y, A, At, tol_fun, tol_x, max_iter, verbose, x0, omega, prox, save_x_interval )

def fista( y, A, At, tol_fun, tol_x, max_iter, verbose, x0, omega, proximal, save_x_interval ) :
    """
    Solve the regularised least squares problem
    ...
    if save_x_interval > 0 :
        coeff_path = self.get_config('COEFF_path')
    res = -y.copy()
    xhat = x0.copy()
