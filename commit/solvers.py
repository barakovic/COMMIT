...
def nnls_nnlasso( y, A, At = None, lambda_v1, index, w, tol_fun = 1e-4, tol_x = 1e-9, max_iter = 100, verbose = 1, x0 = None ) :
    """Solve non negative least squares problem of the following form:

       min 0.5*||y-A x||_2^2 + lambda1*||xEC||_1 s.t. x >= 0
    ...
    if x0 is not None :
        ...
        res = -y
    grad = At.dot(res)
    prev_obj = 0.5 * np.linalg.norm(res)**2 + lambda_v1* np.linalg.norm( res[index:],1 )
    ...
    while True :
        ...
        x = np.real( x )
        x[ x<0 ] = 0
        x[index:] = np.maximum(x[index:] - lambda_v1 * mu, 0)
        # Stepsize check
        tmp = x-xhat
        q = qfval + np.real( np.dot(tmp,grad) ) + 0.5/mu * np.linalg.norm(tmp)**2 + lambda_v1* np.linalg.norm( x[index:],1 )
        res = A.dot(x) - y
        curr_obj = 0.5 * np.linalg.norm(res)**2 + lambda_v1* np.linalg.norm( x[index:],1 )
